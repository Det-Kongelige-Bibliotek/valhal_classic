...
class Person < AuthorityMetadataUnit
  ...
  def self.get_search_objs
    ActiveFedora::SolrService.query("#{self.solr_names[:amu_value]}:* && active_fedora_model_ssi:Person",
        {:rows => ActiveFedora::SolrService.count("#{self.solr_names[:amu_value]}:* && active_fedora_model_ssi:Person")})
  end
