...
module Concerns
  ...
  module GenericFile
    ...
    def add_fits_metadata_datastream(file)
      ...
      logger.info 'Saving the GenericFile Fedora object to be able to call with pid'
      self.save

      logger.info "Putting characterization job on queue for object #{self.pid}."
      Resque.enqueue(CharacterizationJob, self.pid)
    end
